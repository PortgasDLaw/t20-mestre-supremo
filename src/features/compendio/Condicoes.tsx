...
const COR_MAP: Record<string, { badge: string; bg: string; border: string }> = {
  ...
  yellow:   { badge: '#E4C16A', bg: 'rgba(228,193,106,0.10)', border: 'rgba(228,193,106,0.30)' },
  ...
  darkgray: { badge: '#6e6356', bg: 'rgba(110,99,86,0.10)',   border: 'rgba(110,99,86,0.25)' },
...
export default function Condicoes() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Filter */}
      <div className="flex-none flex items-center gap-3 px-8 py-3" style={{ borderBottom: '1px solid rgba(200,155,60,0.10)', background: 'rgba(15,11,19,0.5)' }}>
        <span className="font-cinzel text-xs" style={{ color: '#6e6356', minWidth: 80 }}>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="font-cinzel font-bold text-sm" style={{ color: '#E8DFCF' }}>{condicao.nome}</span>
        <span
      ...
      <div className="space-y-0.5">
        ...
        {condicao.penalidades.length > 2 && (
          <p className="font-cinzel text-[0.6rem]" style={{ color: '#6e6356' }}>+{condicao.penalidades.length - 2} mais...</p>
        )}
      </div>
    </div>
  )
        ...
        <div className="flex-none p-6 pb-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="font-cinzel font-bold text-xl mt-0.5" style={{ color: '#E8DFCF' }}>{condicao.nome}</h2>
            </div>
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded" style={{ color: '#6e6356' }}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-6 pb-6 space-y-4">
          ...
          <div className="rounded-lg p-4" style={{ background: 'rgba(224,80,64,0.10)', border: '1px solid rgba(224,80,64,0.25)' }}>
            ...
            <ul className="space-y-1.5">
              {condicao.penalidades.map((p, i) => (
                <li key={i} className="flex gap-2 font-garamond text-sm" style={{ color: '#E8DFCF' }}>
          ...
          {condicao.interacoes.length > 0 && (
            <div className="rounded-lg p-4" style={{ background: '#120d16', border: '1px solid rgba(200,155,60,0.14)' }}>
              <div className="font-cinzel text-xs uppercase tracking-widest mb-3" style={{ color: '#9a8e7c' }}>Interações</div>
              <ul className="space-y-1.5">
                {condicao.interacoes.map((inter, i) => (
                  <li key={i} className="flex gap-2 font-garamond text-sm" style={{ color: '#a99c86' }}>
                    <span style={{ color: '#C89B3C', flexShrink: 0 }}>◆</span> {inter}
                  </li>
                ))}
              </ul>
            </div>
